...
class Directory:
    ...
    def print_lines(self, index):
        print("Lines {}".format([str(l) for l in self.lines[index] if l.tag is not None]))
    ...
    def read_miss(self, index, tag, p_num):
        ...
        if len(sharers) > 0:
            ...
            if lines[closest].state == CacheState.MODIFIED:
                # Must become shared, causes a coherence writeback
                if self.verbose:
            ...
            # Update directory Lines for requesting processor
    ...
    def write_miss(self, index, tag, p_num):
        ...
        if len(sharers) > 0:
            ...
            for s in sharers:
                ...
                if s == closest:
                    if self.verbose:
                        print("Forward data from P{}.".format(s))
                    if len(sharers) == 1:
                        self.stats.cache_access()
